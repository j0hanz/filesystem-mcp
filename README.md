...
| `FS_ALLOWED_ORIGINS`          | Comma-separated origin hostnames allowed to call `/mcp` from a browser. Replaces the localhost default, so include `localhost` if local browser clients still need access.             |
